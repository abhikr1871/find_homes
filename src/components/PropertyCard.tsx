    ...
    <div className="bg-white rounded-none shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col group border border-gray-200 relative h-full">
      {onToggleFav && (
        <button
          ...
          className="absolute top-4 right-4 bg-white border border-gray-200 p-2 rounded-none shadow-sm hover:bg-gray-50 transition-colors z-10"
      ...
      <Link to={path} className="p-6 flex-1 flex flex-col h-full">
        <div className="flex flex-col mb-4 pr-12">
          <div className="flex flex-wrap gap-2 items-center mb-1.5">
            ...
            {isLive !== undefined && (
              <span className={`shrink-0 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-none ${isLive ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200'}`}>
                {isLive ? 'Live' : 'Off'}
              </span>
            )}
            {badgeStr && (
              <span className="shrink-0 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-none bg-blue-100 text-blue-700 border border-blue-200">
                {badgeStr}
              </span>
            )}
          </div>
        ...
        <div className="mt-auto flex items-end justify-between pt-2">
          ...
          <span className="text-sm font-semibold text-blue-600 border border-blue-100 bg-blue-50 px-4 py-2 rounded-none group-hover:bg-blue-600 group-hover:text-white transition-colors">
            View
          </span>
        </div>
      </Link>
